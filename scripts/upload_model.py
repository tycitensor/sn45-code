...
import argparse
import constants
from coding.model.storage.hugging_face.hugging_face_model_store import HuggingFaceModelStore
from coding.model.model_updater import ModelUpdater
...
async def main(config: bt.config):
    ...
    if not config.skip_model_upload:
        ...
        bt.logging.info(f"Uploading model to Hugging Face with id {model.model_name}")

        await remote_model_store.upload_model(
            model=model,
            local_path=config.model_dir,
        )


        bt.logging.info(f"Model uploaded to Hugging Face {model.model_name} and hash {model.hash}")
    ...
    while True:
        try:
            ...
            await model_metadata_store.store_model_metadata(model)
...
if __name__ == "__main__":
    ...
    if config.list_competitions:
        bt.logging.info(constants.COMPETITION_ID)
    else:
