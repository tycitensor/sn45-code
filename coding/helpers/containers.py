...
class DockerServer:
    ...
    def __init__(self, remote_host_url: str = None, remote_host_registry: str = None):
        ...
        if remote_host_url is not None:
            ...
            try:
                ...
                logging.error("Failed to connect to remote Docker host at %s: %s", remote_host_url, e)
                raise
    ...
    def load_image_remote(self, local_image_tag: str):
        ...
        try:
            ...
            for line in self._local_client.images.push(remote_tag, stream=True, decode=True):
                if 'status' in line:
    ...
    def upload_image_to_dockerhub(self, image: docker.models.images.Image, repository: str, tag: str = "latest"):
        """
        ...
        """
        logging.info(f"Uploading image to DockerHub repository '{repository}' with tag '{tag}'")
        try:
            # Tag the image with the repository and tag
            image.tag(repository, tag)
            
            # Push the image to DockerHub
            for line in self._local_client.images.push(repository, tag, stream=True, decode=True):
                if 'status' in line:
                    logging.info(line['status'])
...
class LocalDockerHandler:
    ...
    def build(self, path: str, tag: str, dockerfile: str = "Dockerfile",
              buildargs: dict = None, rm: bool = True, decode: bool = False, 
              push: bool = False) -> str:
        """
        ...
        try:
            ...
            logging.info(f"Image '{tag}' found in DockerHub, pulling instead of building")
        ...
        try:
            ...
            if push and tag:
                ...
                try:
                    for line in self.client.images.push(repository, tag_part, stream=True, decode=True):
                        if 'status' in line:
                            logging.info(line['status'])
                    logging.info(f"Successfully pushed image to DockerHub: {repository}:{tag_part}")
                except Exception as e:
    ...
    def run(self, image: str, command: str = None, name: str = None,
            ports: dict = None, environment: dict = None, detach: bool = True, **kwargs):
        """
...
class RemoteDockerHandler:
    ...
    def build(self, path: str, tag: str, dockerfile: str = "Dockerfile",
              buildargs: dict = None, rm: bool = True, decode: bool = False, push: bool = False) -> str:
        """
        Builds an image locally, exports it, and then loads it into the remote Docker daemon.
        """
        logging.info("Building image for remote host with tag '%s' from %s", tag, dockerfile)
        try:
            # Build locally using the local handler.
            image = self.server.local.build(path, tag, dockerfile, buildargs, rm, decode, push)
    ...
    def run(self, image: str, command: str = None, name: str = None,
        ...
        try:
            try:
                ...
                logging.info("Image not found on remote host, attempting to transfer from local")
                try:
                    local_image = self.server.local.client.images.get(image)
                    
                    self.server.load_image_remote(local_image.tags[0])
                except Exception as e:
                    logging.error("Error transferring image from local to remote: %s", e)
                    raise
            ...
            logging.info("Container started on remote host with ID %s", container.short_id)
            return container
        except Exception as e:
...
if __name__ == "__main__":
    import os
    REMOTE_DOCKER_URL = os.getenv("REMOTE_DOCKER_HOST")  # Update with your remote host details
